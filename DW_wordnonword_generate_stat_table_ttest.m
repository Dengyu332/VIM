% first created on 09/30
% modified on 10/04/2018: add beta
% just a copy of DW_wordnonword_generate_stat_table.m, but change the
% statistical test to two-sample t-test
% generate stat_table_t.mat under 'VIM/datafiles/preprocessed_new/v2/WordVsNonword/'
    ...
    if ~ismember(contact_id,[13:18]) % treat DBS4039 seperately
        ...
        if length(contact_info(contact_id).session) == 1 % contact having only 1 session
            ...
            for band_id = 1:length(band_selection)
                ...
                [~,p2] = ttest2(p1_word,p1_nonword,'Tail','right');
                [~,p3] = ttest2(p1_word,p1_nonword,'Tail','left');
                ...
                [~,p2] = ttest2(p2_word,p2_nonword,'Tail','right');
                [~,p3] = ttest2(p2_word,p2_nonword,'Tail','left');
                ...
                [~,p2] = ttest2(p3_word,p3_nonword,'Tail','right');
                [~,p3] = ttest2(p3_word,p3_nonword,'Tail','left');
...
save([dionysis 'Users/dwang/VIM/datafiles/preprocessed_new/v2/WordVsNonword/stat_table_t.mat'], 'stat_table','-v7.3');                              