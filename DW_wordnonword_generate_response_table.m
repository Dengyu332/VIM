% first created on 10/02/2018
% Aims to generate speech_response table for each contact's each session,
% each band, each period (3), either trials inclusion (2)
% single-sided permutation test used, no correction.
    ...
    for band_id = 1:length(band_selection)
        ...
        for period_id = 1:length(period_selection)
            ...
            for inclusion_id = 1:length(inclusion_selection)
                ...
                [p2,~,~]= permTest(T_final,base_final,10000,'sidedness', 'larger');
                [p3,~,~] = permTest(T_final,base_final,10000,'sidedness', 'smaller');
...
save([dionysis 'Users/dwang/VIM/datafiles/preprocessed_new/v2/WordVsNonword/speech_response_table.mat'],'speech_response_table','-v7.3');