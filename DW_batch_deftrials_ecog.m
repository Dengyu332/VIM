...
%loop through *_ecog.mat files, chunk raw session into trials
% generate *_session*_ecog_trials_step1.mat
%% set machine
DW_machine;
%% loading packages
...
for Subject_idx = 1:height(Subject_list)
    ...
    data_dir = dir([dionysis,'Users/dwang/VIM/datafiles/preprocessed_new/' Subject_id filesep 'ecog/*_ecog.mat']);
    ...
    for which_session = 1:length(data_dir) % deal with one session at a time
        ...
        D1.state = 'notch_filt,ds2 1khz,lpf_400, hpf_2, trial_chuncked, badch and bad trial not detected';
...
%             filesep data_dir(which_session).name(1:end-4) '_trials_step1_v2.mat'],'D','session_epoch','-v7.3');
        save([dionysis,'Users/dwang/VIM/datafiles/preprocessed_new/' Subject_id filesep 'ecog/'...
             data_dir(which_session).name(1:end-4) '_trials_step1.mat'],'D','session_epoch','-v7.3');       
