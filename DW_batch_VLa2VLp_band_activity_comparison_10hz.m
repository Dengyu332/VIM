...
% generate figures of VLa vs. VLp of different bands under band_region_comparison_10hz_stat
% downsample to 10hz version + did statistical test
            ...
            for locking_id = 1:2
                ...
                switch locking_id
                    case 1 % spct
                        ...
                        for i = 1:size(z_VLa_10hz,1)
                            [p(i),h(i),stat(i)] =  ranksum(z_VLa_10hz(i,:),z_VLp_10hz(i,:));
                        end
                        ...
                        if length(find(p<0.05)) >= 1

                            stat_val = extractfield(stat,'zval');
                        
                            comb = [p;stat_val];                            
                            
                            ap = find(comb(1,:)<0.05 & comb(2,:) > 0); % VLa > VLp

                            pa = find(comb(1,:)<0.05 & comb(2,:) < 0); % VLa < VLp


                            ylims = ylim;

                            y_coor = (ylims(2) - ylims(1)) * 0.99 + ylims(1);

                            hold on; plot(ap *0.1 - 2,repmat(y_coor,1,length(ap)),'LineStyle','none','Marker','*','Color',[0 0.447 0.741]);
                            hold on; plot(pa *0.1 - 2,repmat(y_coor,1,length(pa)),'LineStyle','none','Marker','*','Color',[0.85 0.325 0.098]);
                        ...
                        saveas(gcf,[dionysis 'Users/dwang/VIM/Results/New/v2/band_region_comparison_10hz_stat/'...
                    ...
                    case 2 % cuect
                        ...
                        if length(find(p<0.05)) >= 1
                            ...
                            hold on; plot(ap *0.1 - 1.5,repmat(y_coor,1,length(ap)),'LineStyle','none','Marker','*','Color',[0 0.447 0.741]);
                            hold on; plot(pa *0.1 - 1.5,repmat(y_coor,1,length(pa)),'LineStyle','none','Marker','*','Color',[0.85 0.325 0.098]);
